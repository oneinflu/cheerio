...
router.get('/', auth.requireRole('admin', 'agent', 'supervisor'), async (req, res, next) => {
  try {
    const result = await db.query(
      `
        SELECT id, name, subject, variables, created_at, updated_at
...
router.post('/', auth.requireRole('admin', 'supervisor'), async (req, res, next) => {
  try {
    ...
    const result = await db.query(
      `
        INSERT INTO email_templates (name, subject, html_body, text_body, variables, created_at, updated_at)
        VALUES ($1, $2, $3, $4, $5::jsonb, NOW(), NOW())
        RETURNING id, name, subject, variables, created_at, updated_at
      `,
      [String(name).trim(), String(subject || ''), String(html_body || ''), String(text_body || ''), varsJson]
    );
